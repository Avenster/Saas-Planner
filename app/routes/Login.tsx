...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = isLogin ? "/login1" : "/signup1";
      ...
      if (data.status === "success") {
        ...
        localStorage.setItem("userName", data.name); // This will now be the correct name based on role
      ...
      setError(isLogin ? 'Invalid Credentials' : err.message);
    }
    finally {
      setLoading(false);
    }
              ...
              >
                ...
                {/* {success && (
                  <motion.div
                    ...
                    className="mb-4 p-3 bg-green-500/10 border border-green-500/20 rounded-xl text-green-500 text-sm text-center"
                  >
                    {success}
                  </motion.div>
                )} */}
